...
# e.g.$ Rscript inferSE04.R 3) but off cluster.
Debug = 0; if (Debug & !(runOnCluster)){ participant = 3};                 

if (runOnCluster ==0){
  # Thenk 'participant' must already be in the workspace! 
...
if (!(runOnCluster ==0)){   whoami <- 'MMhal9000';  }
...
tr2natLP4 <- function(trp,check=1){  # from transformed, i.e. -inf to inf, to native space
  ...
  p[,4] <- trp[,4];
  p[,5] <- 0.5*(1 + tanh(trp[,5])) ; # decayCoeffGroups
...
nat2trLP4 <- function(p,check=1){  # From native to transformed.
  ...
  y <- p[,1];        y[y<eps] <- eps; 
  trp[,1] <- log(y);            # ln(a0min-1)
  y <- p[,2]-p[,1]  ;  y[y<eps] <- eps; 
  trp[,2] <- log(y);            # ln(a0max-a0min)
  ...
  trp[,4]  <- p[,4]; #Bpred
  trp[,5]  <- atanh(2*p[,5]-1); #atanh(2decayCoeffGroups-1)
  ...
  if (check){ colnames(trp)<- c('ln(a0min)', 'ln(a0max-a0min)', 'ln(Tpred)', 'Bpred', 'atanh(2decayCoeffGroups-1)', 'atanh(2decayCoeffSelf-1)', 'ln(weightSelf)', 'ln(sensi)', 'ln(sesh)') }
...
try(load(paste(baseDir,"loadfornlm.RData",sep=''))) #this contains bestsofar, tryPmatrix, datArW03, and priors
try(load(paste(baseDir,"/currentWork/loadfornlm.RData",sep=''))) # for kikidi / hal ?
...
beliefModelFit <- function(pts,Par0=NULL,nlmprintlev=0) {
  ...
  ml1fit <- list();
  ml1res <- matrix(NA,nrow=dim(datArW03)[3],ncol=14);
  ...
  for (ptN in pts){ #1:dim(datArW03)[3] ){
    ...
    tryPmatrixwbest[129,] <-bestsofar9[ptN,]
    ...
    mypath <- file.path(paste(fname, "9_1.png", sep = "")) 
    ...
    mypath <- file.path(paste(fname, "9_2.png", sep = "")) 
    ...
    dev.off()
    # And their histogram:
    mypath <- file.path(paste(fname, "9_3.png", sep = "")) 
    png(file=mypath, width = 912, height = 500, units = "px"); 
    hist(log(d2plot[,'obsP']),30,main=paste('pt:',ptN,'  histogram of ln(P(prediction))'),col='gray');
    dev.off()
    ...
    y$ptN = ptN;
    exportCSV(paste(fname,'res9.csv',sep=''),y);
    exportCSV(paste(fname,'evo9.csv',sep=''),ml1fit[[ptN]][[2]][[3]][,,1]);
    save.image(paste(fname,'all9.RData',sep=''));
    if (Debug) {
      ml1fit9 <<- ml1fit9 #so that these objects have global assignment
      ml1res9 <<- ml1res9 #otherwise, only exist within the function
    }
  }
